                                ...
                                <tbody>
                                    {project.invoices.map((inv: any) => (
                                        <tr key={inv.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                                            ...
                                            <td className="p-3 font-mono font-bold text-primary">{inv.quoteNumber || (inv.type === 'QUOTE' ? `Q-${new Date(inv.date).getFullYear()}-${String(inv.number).padStart(3, '0')}` : `INV-${new Date(inv.date).getFullYear()}-${String(inv.number).padStart(3, '0')}`)}</td>
